...
if not defined PYTHON_EXE call :try_python "%USERPROFILE%\Desktop\aplica????es code\WPy64-3.13.12.0\python\python.exe"
...
if not defined PYTHON_EXE (
  ...
  echo %USERPROFILE%\Desktop\aplica????es code\WPy64-3.13.12.0\python\python.exe
  echo.
  pause
  exit /b 1
)
...
if exist "%LOCAL_SCRIPT%" (
  ...
  echo ERRO: monitor_reserva_python_darkjutsu.py nao encontrado na rede nem local.
  echo Rede: %SCRIPT%

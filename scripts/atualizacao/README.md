...
- `automus_update.py`: rotina principal SQL/API + backup + validacoes.
- `automus_config.json`: configuracao local legada (ignorado no git).
- `automus_config.enc.json`: arquivo legado nao usado no modo SQL-only.
...
4) chama `atualizacao.automus_update` para atualizar o estoque via SQL/API.

Compatibilidade:
- `scripts/automus_update.py` foi mantido como wrapper.

Build do exe:
- O `build_automus_exe.py` gera pacote SQL-only e nao empacota credenciais legadas.

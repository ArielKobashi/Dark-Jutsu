...
# Macro 6: envio das 3 planilhas para o Dark Jutsu e clique em "Atualizar Banco".
...
def _resolve_target_file(base: Path, filename: str) -> Path:
    ...
    for path in candidates:
        if path.exists() and path.is_file():
            return path
    raise FileNotFoundError(f"Arquivo nao encontrado para envio no Dark Jutsu: {filename}")
